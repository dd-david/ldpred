...
#        sys.exit(2)
    ...
    # 입력 파라미터의 기본값으로 세팅되는 부분들이 있습니다.
    # --ssf_format : 'STANDARD' 포맷의 GWAS summary statistics 를 다룹니다.
    # --gf_format  : 'PLINK' 포맷의 genotypes 데이터를 다룹니다.
    # --maf        : 0.01의 minor allele frequency 를 세팅합니다.
...
# GWAS summary statistics 와 .bim 파일을 입력으로 받습니다.
# 두 파일에서 교집합을 이루는 SNPs 을 추려냅니다.
# 추려낸 SNPs을 chromosome 별로, HDF5 포맷으로 저장합니다.
#
# @parameters
# filename=p_dict['ssf'] : GWAS summary statistics 파일
# bimfile=bimfile : .bim 파일
# hdf5_file=h5f : 출력할 HDF5 파일의 레퍼런스 (*)
# n=p_dict['N'] : 샘플개수
#  (dd-david) 샘플개수는 어디에도 사용되지 않는데?
...
#                              'betas':[], 'nts': [], 'sids': [], 'positions': []}
    ...
    # {
    #   group      /sum_stats
    #   group      /sum_stats/chrom_1
    #   dataset    /sum_stats/chrom_1/betas
    #   dataset    /sum_stats/chrom_1/freqs
    #   dataset    /sum_stats/chrom_1/infos
    #   dataset    /sum_stats/chrom_1/log_odds
    #   dataset    /sum_stats/chrom_1/nts
    #   dataset    /sum_stats/chrom_1/positions
    #   dataset    /sum_stats/chrom_1/ps
    #   dataset    /sum_stats/chrom_1/sids
    # }
    ...
    for chrom in chrom_dict.keys():
        ...
        # chromosome 마다, SNP당 각각의 tuple을 만들어내는 작업을 합니다.
        # 정렬을 통해서 'position' 기준으로 오름차순 정렬을 합니다.
        # [(1, ...), (2, ...), ... ]
        ...
        # chromosome 별, SNPs('position' 기준 오름차순 정렬된) 들을 항목별로 나눠서 저장합니다.
        # (예외처리) 'position'이 같은 중복 SNP은 제거합니다.
        for pos, sid, nt, beta, lo, info, frq, p in sl:
            # 'position' 이 중복되는게 있는지
            if pos == prev_pos:
...
def coordinate_genot_ss(genotype_file=None,
    ...
    #    dataset    /fids
    #    dataset    /iids
    #    -----------------------(아래는 GWAS summary statistics 리딩과정에서 세팅됨)
    #    group      /sum_stats
    #    group      /sum_stats/chrom_1
    #    dataset    /sum_stats/chrom_1/betas
    #    dataset    /sum_stats/chrom_1/freqs
    ...
    # .bim 파일의 형태 (__ get_loci)
    # chrom   rsId        cM   bp   allele1  allele2
    #     1	 sid_0	0.000000	0	      A	       G
    #     1	 sid_1	0.000000	1	      A	       G
    # -----  -----
    loci = plinkf.get_loci()
    plinkf.close()

    # bim 파일에 존재하는 chromosomes 리스트를 추려냅니다. [1...22]
...
#         ok_indices['ss'] = list(sp.array(ok_indices['ss'])[order])
        ...
        raw_ref_snps, freqs_ref = plinkfiles.parse_plink_snps(reference_genotype_file, snp_indices_ref)
...
#         assert sp.all(g_sids==rg_sids) and sp.all(rg_sids==ss_sids), 'WTF?'
        ...
        if plinkf_dict['has_phenotype']:
            maf_adj_corr = sp.corrcoef(plinkf_dict['phenotypes'], maf_adj_prs)[0, 1]
...
#         print 'Sum betas', sp.sum(betas ** 2)
    ...
    # Now calculate the prediction r^2
    if plinkf_dict['has_phenotype']:
        maf_adj_corr = sp.corrcoef(plinkf_dict['phenotypes'], maf_adj_risk_scores)[0, 1]
...
def main():
    ...
    # coord_genotypes 의 산출물인 HDF5 포맷 파일을 생성합니다.
    h5f = h5py.File(p_dict['out'], 'w')

    # GWAS summary statistics 파일의 타입에 맞춰서 파싱합니다.
    # HDF5 포맷에 chromosomes 별로 구분된 SNPs 데이터세트를 얻을 수 있습니다.
    # (SNPs, GWAS summaray statistics 와 .bim 파일의 교집합)
    ...
    # Genotype 포맷을 확인해서, genotypes와 GWAS summuary statistics 파일을 coordinate 합니다.
    else:
